...
class ExpensesController extends AppController
{
    ...
    public function index($user_id = null)
    {
        ...
        if ($user_id != null) {
            if ($user_id !== null && count($token) > 0) {
                ...
                if ($user->token != null) {
                    if ($user->token == $token[0]) {
                        $expenses = $this->Expenses->find()->where(['user_id' => $user_id])->all();
            ...
            return $this->response->withStatus(401)->withStringBody(json_encode(['Not Authorized']));
        } else {
            return $this->response->withStatus(404)->withStringBody(json_encode(['User has not be found']));
        }
    }
        ...
        if ($user_id != null || $id != null) {
            if ($user_id !== null && count($token) > 0) {
                ...
                if ($user->token != null) {
                    if ($user->token == $token[0]) {
                        try {
                            $expenses = $this->Expenses->find()->where(['user_id' => $user_id, 'id' => $id])->firstOrFail();
